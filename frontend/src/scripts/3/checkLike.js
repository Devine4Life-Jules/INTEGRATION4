...
const checkLike = (hand) => {
  ...
  const isThumbUp = hand.thumb_tip.y < hand.thumb_ip.y;

  // Check if other fingers are folded (tips below their respective PIP joints)
  let indexFolded, isMiddleFolded, isRingFolded, isPinkyFolded;

  if (hand.handedness === "Right") {
    indexFolded = hand.index_finger_tip.x > hand.index_finger_pip.x; // Compare to index PIP
  ...
  const thumbAboveWrist = hand.thumb_tip.y < hand.wrist.y;
